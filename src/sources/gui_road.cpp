...
GUI_Road::boundingRect () const
{
  ...
  offset2 = QPointF (0, 0);
  nPolygon << line.p1 () + offset1 << line.p1 () + offset2
...
GUI_Road::getCenter ()
{
  return QPointF (
      m_start->CenterPosition ().rx ()
          - (m_start->CenterPosition ().rx () - m_end->CenterPosition ().rx ())
                / 2.0,
...
// setters
void
GUI_Road::set_pen (QColor color)
{
  m_pen = QPen (color);
}

void
GUI_Road::paint (QPainter *painter, const QStyleOptionGraphicsItem *option,
                 QWidget *widget)
{
  Q_UNUSED (option);
  Q_UNUSED (widget);
