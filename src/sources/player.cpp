...
Player::Player(int id, std::string name, QColor color, QColor color_c /*, Color color*treba ovo da se doradi*/)
    : m_id(id), m_name(std::move(name)), m_victory_points(0), m_num_of_roads(0), m_num_of_houses(0), m_num_of_cities(0),
      m_num_of_allowed_houses(5), m_num_of_allowed_cities(4), m_num_of_allowed_roads(15),
      m_player_color(std::move(color)), m_city_color(std::move(color_c))
{
...
void Player::dec_num_of_brick()
{
    m_player_resource[ResourceType::Brick] -= 1;
    m_num_of_brick--;
    ; // kada se ovo zakomentarise onda ne prikazuje da je oduzeto tako da ova linija je vezana za duplo oduzimanje
}
