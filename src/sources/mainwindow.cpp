...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), m_board(new Board(this)), m_dice(new Dice())
...
void MainWindow::on_pb_MagicCard_clicked()
{
    if (m_dice->get_dice_is_rolled() == true)
    {
        if (m_game->can_get_magic_card())
        {
            ...
            {
                QMessageBox msgBox;
                msgBox.setText("CONGRATS " + QString::fromStdString(m_game->getCurrentPlayer()->get_name()) +
                               ", YOU WON");
                int ret = msgBox.exec();
                if (ret == QMessageBox::Ok)
                {
