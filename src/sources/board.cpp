...
void
Board::mousePressEvent (QGraphicsSceneMouseEvent *event)
{

  if (m_setRoad
      && itemAt (event->scenePos (), QTransform ()) /*->type() == 1*/)
    {
      if (!m_hasTmp)
        {
          this->m_tmp = dynamic_cast<GUI_Node *> (
        ...
        {
          GUI_Node *node = dynamic_cast<GUI_Node *> (
              itemAt (event->scenePos (), QTransform ()));
  ...
  if (m_setHouse
    ...
    {
      Redraw ();
      GUI_Node *gui_node = dynamic_cast<GUI_Node *> (
          itemAt (event->scenePos (), QTransform ()));
...
void
Board::setHasTmp (bool x)
{
  m_hasTmp = x;
}

void
Board::setCurrColor (QColor color)
{
  m_curr_color = color;
}
...
void
Board::setRoadColor (QColor color)
{
  m_road_color = color;
}
...
bool
Board::is_first_turn () const
{
  return m_first_turn;
}

void
Board::set_first_turn (bool value)
{
  m_first_turn = value;
}
...
Board::addAllFields ()
{
  ...
  const auto Field12
      = new Field (12, 11, m_nodes[22], m_nodes[24], m_nodes[37], m_nodes[36],
                   m_nodes[35], m_nodes[24]);
  ...
  const auto Field15
      = new Field (15, 11, m_nodes[31], m_nodes[32], m_nodes[33], m_nodes[44],
                   m_nodes[43], m_nodes[42]);
  ...
  const auto Field18
      = new Field (18, 10, m_nodes[41], m_nodes[42], m_nodes[43], m_nodes[51],
                   m_nodes[50], m_nodes[49]);
