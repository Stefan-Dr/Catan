...
void Board::addAllNodes()
{
    ...
    {
        connect(
            m_gui_nodes[i], &GUI_Node::needRedraw, this,
            &Board::Redraw); // slicno ovom hoveru i signalu treba da se uradi sa iscrtavanjem kuce odredjenog igraca

        // tj. u fji set house/ city se baca signal needRedraw
        // ali bitna napomena, sva crtanja kuca/grad itd se rade u okviru mousepressevent funkcije, samo se razlicito
        // pisu uslovi i to je to
        addItem(m_gui_nodes[i]);
    }
}
...
void Board::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (m_setCity && (itemAt(event->scenePos(), QTransform()) /*->type()==1*/))
    {
        Redraw();
        GUI_Node *node = dynamic_cast<GUI_Node *>(itemAt(event->scenePos(), QTransform()));
        // provera da li postoji kuca i da li ta kuca pripada igracu na potezu ili da li je vec izgradjen grad na tom
        // cvoru
...
void Board::addAllFields()
{
    ...
    const auto Field12 =
        new Field(12, 11, m_nodes[22], m_nodes[24], m_nodes[37], m_nodes[36], m_nodes[35], m_nodes[24]);
    ...
    const auto Field15 =
        new Field(15, 11, m_nodes[31], m_nodes[32], m_nodes[33], m_nodes[44], m_nodes[43], m_nodes[42]);
    ...
    const auto Field18 =
        new Field(18, 10, m_nodes[41], m_nodes[42], m_nodes[43], m_nodes[51], m_nodes[50], m_nodes[49]);
