...
QVector<Field *> Board::get_fields() { return m_fields; }

void Board::addAllNodes() {

  /*prvi heksagon prvi red */

  // gornji levi ugao prvog heksagona njegovi susedi su - prvi ispod njega po y
  // osi (185,130) i vrh heksagona (250,20)
  ...
  //    addItem(gnode3);
  for (int i = 0; i < 54; i++) {
    connect(m_gui_nodes[i], &GUI_Node::needRedraw, this,
            &Board::Redraw); // slicno ovom hoveru i signalu treba da se uradi
                             // sa iscrtavanjem kuce odredjenog igraca

    // tj. u fji set house/ city se baca signal needRedraw
    // ali bitna napomena, sva crtanja kuca/grad itd se rade u okviru
    // mousepressevent funkcije, samo se razlicito pisu uslovi i to je to
    addItem(m_gui_nodes[i]);
  }
}
...
void Board::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if (m_setRoad && itemAt(event->scenePos(), QTransform()) /*->type() == 1*/) {
    if (!m_hasTmp) {
      this->m_tmp =
          dynamic_cast<GUI_Node *>(itemAt(event->scenePos(), QTransform()));
      // Node* node = m_tmp->getNode();
      if (this->m_tmp->get_is_end_of_road()) {
        setHasTmp(true);
      } else {
        emit invalidRoad();
      }
  ...
  if (m_setHouse && (itemAt(event->scenePos(), QTransform()) /*->type()==1*/)) {
    ...
    if (!gui_node->get_is_house_built() && neighbours_are_occupied == 0) {
      ...
      if (getCurrColor() == Qt::blue) {
        gui_node->getNode()->set_owner(1);
      } else if (getCurrColor() == Qt::yellow) {
        gui_node->getNode()->set_owner(2);
      } else if (getCurrColor() == Qt::green) {
        gui_node->getNode()->set_owner(3);
      } else {
        gui_node->getNode()->set_owner(4);
      }
    } else {
      emit invalidHouse();
    }
  ...
  if (m_setCity && (itemAt(event->scenePos(), QTransform()) /*->type()==1*/)) {
    ...
    GUI_Node *node =
        dynamic_cast<GUI_Node *>(itemAt(event->scenePos(), QTransform()));
    // provera da li postoji kuca i da li ta kuca pripada igracu na potezu ili
    // da li je vec izgradjen grad na tom cvoru
    if (node->get_is_house_built() && !(node->get_is_city_built()) &&
        node->check_owner_city(getCurrColor())) {
...
void Board::Redraw() { this->update(); }

void Board::setHasTmp(bool x) { m_hasTmp = x; }

void Board::setCurrColor(QColor color) { m_curr_color = color; }

QColor Board::getCurrColor() { return m_curr_color; }

void Board::setRoadColor(QColor color) { m_road_color = color; }

QColor Board::getRoadColor() { return m_road_color; }

bool Board::is_first_turn() const { return m_first_turn; }

void Board::set_first_turn(bool value) { m_first_turn = value; }

void Board::addAllFields() {
  addAllNodes();
  // prvi red
  const auto Field1 = new Field(1, 10, m_nodes[0], m_nodes[1], m_nodes[2],
                                m_nodes[3], m_nodes[4], m_nodes[5]);
  Field1->set_res_type(ResourceType::Stone);
  m_fields.push_back(Field1);

  const auto Field2 = new Field(2, 8, m_nodes[2], m_nodes[6], m_nodes[7],
                                m_nodes[8], m_nodes[9], m_nodes[3]);
  Field2->set_res_type(ResourceType::Brick);
  m_fields.push_back(Field2);

  const auto Field3 = new Field(3, 3, m_nodes[7], m_nodes[10], m_nodes[11],
                                m_nodes[12], m_nodes[13], m_nodes[8]);
  Field3->set_res_type(ResourceType::Wood);
  m_fields.push_back(Field3);

  // drugi red
  const auto Field4 = new Field(4, 2, m_nodes[14], m_nodes[5], m_nodes[4],
                                m_nodes[17], m_nodes[16], m_nodes[15]);
  Field4->set_res_type(ResourceType::Wheat);
  m_fields.push_back(Field4);

  const auto Field5 = new Field(5, 3, m_nodes[4], m_nodes[3], m_nodes[9],
                                m_nodes[19], m_nodes[18], m_nodes[17]);
  Field5->set_res_type(ResourceType::Wool);
  m_fields.push_back(Field5);

  const auto Field6 = new Field(6, 9, m_nodes[9], m_nodes[8], m_nodes[13],
                                m_nodes[21], m_nodes[20], m_nodes[19]);
  Field6->set_res_type(ResourceType::Brick);
  m_fields.push_back(Field6);

  const auto Field7 = new Field(7, 4, m_nodes[13], m_nodes[12], m_nodes[23],
                                m_nodes[24], m_nodes[22], m_nodes[21]);
  Field7->set_res_type(ResourceType::Brick);
  m_fields.push_back(Field7);

  // treci red
  const auto Field8 = new Field(8, 12, m_nodes[25], m_nodes[15], m_nodes[16],
                                m_nodes[28], m_nodes[27], m_nodes[26]);
  Field8->set_res_type(ResourceType::Wool);
  m_fields.push_back(Field8);

  const auto Field9 = new Field(9, 8, m_nodes[16], m_nodes[17], m_nodes[18],
                                m_nodes[30], m_nodes[29], m_nodes[28]);
  Field9->set_res_type(ResourceType::Stone);
  m_fields.push_back(Field9);

  const auto Field10 = new Field(10, 4, m_nodes[18], m_nodes[19], m_nodes[20],
                                 m_nodes[32], m_nodes[31], m_nodes[30]);
  Field10->set_res_type(ResourceType::Stone);
  m_fields.push_back(Field10);

  const auto Field11 = new Field(11, 6, m_nodes[20], m_nodes[21], m_nodes[22],
                                 m_nodes[34], m_nodes[33], m_nodes[32]);
  Field11->set_res_type(ResourceType::Wood);
  m_fields.push_back(Field11);

  const auto Field12 = new Field(12, 11, m_nodes[22], m_nodes[24], m_nodes[37],
                                 m_nodes[36], m_nodes[35], m_nodes[24]);
  Field12->set_res_type(ResourceType::Wheat);
  m_fields.push_back(Field12);

  // cetvrti red
  const auto Field13 = new Field(13, 9, m_nodes[27], m_nodes[28], m_nodes[29],
                                 m_nodes[40], m_nodes[39], m_nodes[38]);
  Field13->set_res_type(ResourceType::Wheat);
  m_fields.push_back(Field13);

  const auto Field14 = new Field(14, 5, m_nodes[29], m_nodes[30], m_nodes[31],
                                 m_nodes[42], m_nodes[41], m_nodes[40]);
  Field14->set_res_type(ResourceType::Wood);
  m_fields.push_back(Field14);

  const auto Field15 = new Field(15, 11, m_nodes[31], m_nodes[32], m_nodes[33],
                                 m_nodes[44], m_nodes[43], m_nodes[42]);
  Field15->set_res_type(ResourceType::Wool);
  m_fields.push_back(Field15);

  const auto Field16 = new Field(16, 5, m_nodes[33], m_nodes[34], m_nodes[35],
                                 m_nodes[46], m_nodes[45], m_nodes[44]);
  Field16->set_res_type(ResourceType::Wood);
  m_fields.push_back(Field16);

  // peti red
  const auto Field17 = new Field(17, 6, m_nodes[39], m_nodes[40], m_nodes[41],
                                 m_nodes[49], m_nodes[48], m_nodes[47]);
  Field17->set_res_type(ResourceType::Wheat);
  m_fields.push_back(Field17);

  const auto Field18 = new Field(18, 10, m_nodes[41], m_nodes[42], m_nodes[43],
                                 m_nodes[51], m_nodes[50], m_nodes[49]);
  Field18->set_res_type(ResourceType::Wheat);
  m_fields.push_back(Field18);

  // poslednje polje je pustinja pa cu staviti da je na njemu broj = 0,takodje
  // nema tip resursa
  const auto Field19 = new Field(19, 0, m_nodes[43], m_nodes[44], m_nodes[45],
                                 m_nodes[53], m_nodes[52], m_nodes[51]);
